...
class KtlintPluginVersionTest : AbstractPluginTest() {
    ...
    @DisplayName("Should allow to use KtLint version 1.0.1")
    @CommonTest
    fun allowSupportedMinimalKtLintVersion(gradleVersion: GradleVersion) {
        project(gradleVersion) {
            withCleanSources()
            buildGradle.useKtlintVersion("1.0.1")
    ...
    @DisplayName("Should fail the build on using KtLint version <1")
    @CommonTest
    fun failOnUnsupportedOldKtLintVersion(gradleVersion: GradleVersion) {
        project(gradleVersion) {
            withCleanSources()
            buildGradle.useKtlintVersion("0.50.0")
