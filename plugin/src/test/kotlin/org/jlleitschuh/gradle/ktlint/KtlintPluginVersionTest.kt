...
class KtlintPluginVersionTest : AbstractPluginTest() {
    ...
    @DisplayName("Should allow to use KtLint version 0.45.2")
    @CommonTest
    fun allowSupportedMinimalKtLintVersion(gradleVersion: GradleVersion) {
        project(gradleVersion) {
            withCleanSources()
            buildGradle.useKtlintVersion("0.45.2")
    ...
    @DisplayName("Should fail the build on using KtLint version <0.45.2")
    @CommonTest
    fun failOnUnsupportedOldKtLintVersion(gradleVersion: GradleVersion) {
        project(gradleVersion) {
            withCleanSources()
            buildGradle.useKtlintVersion("0.45.1")
